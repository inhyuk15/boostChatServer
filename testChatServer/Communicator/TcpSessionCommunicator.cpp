...
boost::asio::awaitable<ChatMessageWrapper> TcpSessionCommunicator::asyncRead() {
    ...
    co_await boost::asio::async_read(
        socket_, boost::asio::buffer(&networkDataSize, sizeof(networkDataSize)),
        use_awaitable);

    std::size_t dataSize = ntohl(networkDataSize);

    std::string binaryData(dataSize, '\0');
    co_await boost::asio::async_read(socket_, boost::asio::buffer(binaryData),
                                     use_awaitable);
...
boost::asio::awaitable<void> TcpSessionCommunicator::asyncWrite(
    ...
    co_await boost::asio::async_write(socket_, boost::asio::buffer(sendBytes),
                                      use_awaitable);
...
void TcpSessionCommunicator::cancelOne() { timer_.cancel_one(); }

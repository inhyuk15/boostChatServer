...
boost::asio::awaitable<ChatMessageWrapper> WsSessionCommunicator::asyncRead()
{
    ...
    const std::string binaryData = boost::beast::buffers_to_string(buffer.data());
...
boost::asio::awaitable<void> WsSessionCommunicator::asyncWrite(const std::string &sendBytes)
{
    co_await socket_.async_write(boost::asio::buffer(sendBytes), boost::asio::use_awaitable);
    co_return;
}
...
void WsSessionCommunicator::cancelOne()
{
    timer_.cancel_one();
}
...
{
    socket_.async_close(boost::beast::websocket::close_reason(boost::beast::websocket::close_code::normal),
